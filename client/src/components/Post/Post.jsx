...
function Post(props) {
  ...
  const likeClicked = async () => {
    try {
      axios.put(
        `${process.env.React_App_PUBLIC_URL}/post/${props.post._id}/like/`,
        {
          userId: cuser._id,
        }
      );
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      const res = await axios.get(
        `${process.env.React_App_PUBLIC_URL}/user?userId=${props.post.userId}`
      );
      setUser(res.data);
    };
    getUser();
  ...
  const deletePost = () => {
    ...
    axios
      .delete(
        `${process.env.React_App_PUBLIC_URL}/post/${props.post._id}?userId=${cuser._id}`
      )
